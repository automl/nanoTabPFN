...
def train(
    ...
    logs to wandb, and prints diagnostics (Dormant Neuron Ratio, Effective Rank, Grad "conflict").
    """
    if not device:
    ...
    # track gradient "conflict" vs previous batch
    prev_grad = None
    ...
    try:
        for step, full_data in enumerate(prior):
            ...
            if diag_every and (step % diag_every == diag_every - 1):
                ...
                dnr_str = (
                    " | ".join(
                        [f"L{li}: {r:.2f}" for li, r in sorted(dnr_ratios.items())]
                    )
                    if dnr_ratios
                    else "n/a"
                )
                print(
                    f"[diag] step {step+1} | time {train_time:6.1f}s "
                    f"| loss_total {float(loss.detach().cpu()):.4f} "
                    f"| CE {float(ce_loss.detach().cpu()):.4f} "
                    f"| DNR {dnr_str} "
                    f"| EffRank {eff_rank if eff_rank is not None else 'n/a'}"
                    f"{f' | GradDotPrev {grad_cos:.4f}' if grad_cos is not None else ''}"
                )

                # wandb logging for diagnostics
                diag_log = {
                    "step": step,
                    "train/time": train_time,
                    "train/loss_total": float(loss.detach().cpu()),
                    "train/loss_ce": float(ce_loss.detach().cpu()),
                }
                for li, r in dnr_ratios.items():
                    diag_log[f"diag/dnr_layer_{li}"] = r
                if eff_rank is not None:
                    diag_log["diag/effective_rank"] = eff_rank
                if grad_cos is not None:
                    diag_log["diag/grad_dot_prev"] = grad_cos
            ...
            if step % steps_per_eval == steps_per_eval - 1 and eval_func is not None:
                ...
                score_str = " | ".join(
                    [f"{k} {v:7.4f}" for k, v in scores.items()]
                )
                print(
                    f"time {train_time:7.1f}s | loss_total {float(loss.detach().cpu()):7.4f} "
                    f"| CE {float(ce_loss.detach().cpu()):7.4f} | {score_str}"
                )

                # wandb logging for eval metrics
                log_data = {
                    "step": step,
                    "train/time": train_time,
                    "train/loss_total": float(loss.detach().cpu()),
                ...
                print(
                    f"time {train_time:7.1f}s | loss_total {float(loss.detach().cpu()):7.4f} "
                    f"| CE {float(ce_loss.detach().cpu()):7.4f}"
                )
...
if __name__ == "__main__":
    ...
    prior1 = PriorDumpDataLoader("300k_150x5_2.h5", num_steps=2500, batch_size=32, device=device)
    ...
    prior2 = PriorDumpDataLoader("300k_150x5_2.h5", num_steps=5000, batch_size=32, device=device)
    ...
    wandb.log({f"final/{k}": v for k, v in final_scores.items()})
    wandb.finish()
